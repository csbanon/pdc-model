...
public class Graph
{
  ...
  // Returns the degree distibution of the graph.
  ...
  // Returns the number of mutual edges between nodes A and B.
  public int getNumMutualEdges(Node A, Node B)
  {
    int numMutualEdges = 0;

    for (Node node : adjacencyList.get(A))
    {
      if (adjacencyList.get(B).contains(node))
      {
        numMutualEdges++;
      }
    }

    return numMutualEdges;
  }
